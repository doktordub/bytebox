...
def backup_arcade_database(
    ...
    *,
    ...
    if destination is None:
        ...
        destination_path = database_path.parent / "backups" / f"{database_path.name}-{timestamp}"
    else:
        destination_path = normalize_database_path(destination)

    if destination_path.exists():
        if not overwrite:
            raise PersistenceError(f"Backup destination already exists: {destination_path}")
        _remove_path(destination_path)

    destination_path.parent.mkdir(parents=True, exist_ok=True)
    _copy_path(database_path, destination_path)
    return {
...
def restore_arcade_database(
    ...
    *,
    ...
    database_path = normalize_database_path(path)
    source_path = normalize_database_path(backup_path)
    if not source_path.exists():
        raise PersistenceError(f"ArcadeDB backup does not exist: {source_path}")

    if database_path.exists():
        if not overwrite:
            raise PersistenceError(
                f"ArcadeDB database already exists at {database_path}; rerun with overwrite=True."
            )
        _remove_path(database_path)

    database_path.parent.mkdir(parents=True, exist_ok=True)
    _copy_path(source_path, database_path)
    return {
        "database_path": str(database_path),
